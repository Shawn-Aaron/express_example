 ...
 * @param {stirng} sqltext 查询数据的sql语句
 * @param {function} func 回调函数 共有三个参数 error:错误消息 recordsets:查询的结果 affected:影响的行数
 */
sql.queryByPage=function(page,size,orderBy,sqltext,func){
	var pageOrderBy = orderBy == "" ? "(select 0)":orderBy;
	var beginIndex = (page - 1) * size + 1;
	var endIndex = parseInt(beginIndex) + parseInt(size) - 1;
	var pageSql = "with query as (select ROW_NUMBER() over(order by " + pageOrderBy + ") AS ROWNUM, a.* FROM ("
		+ sqltext + ") a) select * from query where ROWNUM BETWEEN " + beginIndex + " AND " + endIndex;
	sql.queryWithParams(pageSql,null,func);
};
